...
class GetEatMinxin(viewsets.ModelViewSet):
    paginate_by = 25
    filter_backends = (
        django_filters.rest_framework.DjangoFilterBackend,
        filters.SearchFilter,
...
class EatViewSet(DefaultsMixin):

    queryset = Eat.objects.all().order_by('-date_created')
...
class GetEatViewSet(GetEatMinxin):

    queryset = Eat.objects.all().order_by('-date_created')
    serializer_class = QuickNewSerializer

    def list(self, request, *args, **kwargs):
        eat = self.queryset.all()
        page = self.paginate_queryset(eat)
        serializer = self.get_serializer(page, many=True)
        return self.get_paginated_response(serializer.data)